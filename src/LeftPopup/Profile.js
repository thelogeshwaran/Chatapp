...
function Profile() {
    ...
    const { setLeftPopup,setSelected, selected } = usePopupProvider();
        ...
        <div className="profile">
             <Menu
                ...
                >
                    <form>
                        <label>
                            <input className="profile__imageinput" type="file" onChange={inputHandler}></input>
                            <MenuItem onClick={handleClose}>Change Photo</MenuItem>
                        </label>
                        <MenuItem onClick={()=> {handleView(); handleClose();}}>View photo</MenuItem>
                        
                        <MenuItem onClick={ ()=> {handleClose(); handleDelete();}}>Remove photo</MenuItem>
                    </form>
                    
                </Menu>
            <div className="profile__header">
               <KeyboardBackspaceIcon onClick={()=> setLeftPopup("")} style={{cursor:"pointer"}}/>
               <h2>Profile</h2>
           </div>
            <div className="profile__avatar">
            <Avatar alt="Remy Sharp" src={imageUrl} style={{ height:"200px", width:"200px"}} aria-controls="simple-menu" aria-haspopup="true" onClick={handleClick}/>
            </div>
            ...
            <div className="profile__info">
                ...
                <input value={name} onChange={(e)=> setName(e.target.value)} disabled={disabled} style={ { borderBottom : disabled ? "none" : "1px solid #00BFA5" }}></input>
                {
                    ...
                    <IconButton onClick={()=> {setDisabled(true); handleSubmit();}}><CheckIcon/></IconButton>
                }
                </div>
            </div>
