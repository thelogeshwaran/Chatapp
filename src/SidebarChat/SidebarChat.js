    ...
    return (
        ...
        <div className="sidebarChat">
            ...
            <div className="sidebarChat__info">
                ...
                <p>{messages[0] ? messages[0]?.message?.type==="text" ? messages[0]?.message.message.substring(0, 15) + "..." : <CameraAltIcon style={{ color:'grey'}}/> : "" }</p>
            </div>
            </div>
            <div className="sidebarChat__time">
                {messages[0] ? new Date(messages[0]?.timeStamp?.toDate()).toLocaleTimeString() : ""}
            </div>
        </div>
        </Link>
    )   
